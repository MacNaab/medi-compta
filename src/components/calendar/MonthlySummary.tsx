...
export function MonthlySummary({ journees, currentDate, lieux, parseLocalDate }: MonthlySummaryProps) {
  ...
  const monthJournees = journees.filter(j => {
    ...
    const matchesFilter = selectedLieuId === 'all' || j.lieuId === selectedLieuId;
    return isInMonth && matchesFilter;
  });

  const totalRecettes = monthJournees.reduce((sum, j) => sum + (j.recettesTotales || 0), 0);
  const totalHonoraires = monthJournees.reduce((sum, j) => sum + (j.honorairesTheoriques || 0), 0);
    ...
    <div className="p-4 border-t border-border bg-muted/30 space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-6">
          ...
          <div className="text-sm">
            ...
            <span className="font-semibold">{totalRecettes.toLocaleString('fr-FR')} €</span>
          </div>
          <div className="text-sm">
            <span className="text-muted-foreground">Honoraires : </span>
            <span className="font-semibold text-primary">{totalHonoraires.toLocaleString('fr-FR')} €</span>
          </div>
        </div>
      ...
      {nbJournees > 0 && (
        <div className="text-xs text-muted-foreground flex items-center gap-2">
          ...
          <span>
            Moyenne par journée : {(totalRecettes / nbJournees).toLocaleString('fr-FR', { maximumFractionDigits: 0 })} € recettes / {(totalHonoraires / nbJournees).toLocaleString('fr-FR', { maximumFractionDigits: 0 })} € honoraires
          </span>
        </div>
      )}
    </div>
