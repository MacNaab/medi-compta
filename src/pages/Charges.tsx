...
export default function Charges() {
  ...
  const stats = useMemo(() => {
    ...
    const deductible = filteredCharges
      .filter((c) => c.deductible)
      .reduce((sum, c) => sum + c.montant, 0);
    const byCategory = CATEGORIES.map((cat) => ({
      ...cat,
      total: filteredCharges
        .filter((c) => c.categorie === cat.value)
        .reduce((sum, c) => sum + c.montant, 0),
    })).filter((c) => c.total > 0);
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-xs text-muted-foreground">
              {filteredCharges.length} dépense
              {filteredCharges.length > 1 ? "s" : ""} en {selectedYear}
            </p>
          </CardContent>
        </Card>
        <Card>
                ...
                {filteredCharges.map((charge) => {
                  ...
                  return (
                    <TableRow key={charge.id}>
                      <TableCell>
                        {format(new Date(charge.date), "dd/MM/yyyy")}
                      </TableCell>
                      <TableCell>
                        <div className="flex items-center gap-2">
          ...
          <div className="space-y-4 py-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    setForm({ ...form, categorie: v as CategoryValue })
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
            ...
            <div className="space-y-2">
              ...
              <NumberInput
                ...
                onValueChange={(e) =>
                  setForm({ ...form, montant: e.toString() })
                }
              />
            </div>
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-muted-foreground">
                  À inclure dans la déclaration fiscale
                </p>
              </div>
              <Switch
                checked={form.deductible}
                onCheckedChange={(checked) =>
                  setForm({ ...form, deductible: checked })
                }
              />
            </div>
            <div className="space-y-2">
