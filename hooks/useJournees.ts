...
export function useJournees() {
  ...
  const createJournee = async (data: JourneeFormData): Promise<Journee> => {
    ...
    await loadJournees();
    await insertToCloud("journees", journeeCree);
    return journeeCree;
  };

  const updateJournee = async (
    id: string,
    data: Partial<JourneeFormData>
  ): Promise<Journee> => {
    ...
    if (data.recettesTotales !== undefined || data.lieuId !== undefined) {
      ...
      if (lieu) {
        const recettes =
          data.recettesTotales !== undefined
    ...
    await loadJournees();
    await updateToCloud("journees", journeeModifiee);
  ...
  const deleteJournee = async (id: string): Promise<void> => {
    ...
    await loadJournees();
    await deleteToCloud("journees", id);
  };
  ...
  const getJourneesFiltrees = (filtres: {
    ...
    if (filtres.annee) {
      result = result.filter(
        (j) => new Date(j.date).getFullYear() === filtres.annee
      );
    }
